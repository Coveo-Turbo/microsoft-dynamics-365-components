...
export class InsightContext extends PipelineContext {
     ...
     */
    public constructor(public element: HTMLElement, public options: IInsightContextOptions, public bindings: IComponentBindings) {
        super(element, InsightContext.ID, bindings);
        this.options = ComponentOptions.initComponentOptions(element, InsightContext, options);
        ...
        if (this.options.fieldsToInclude) {
            this.bind.onRootElement(QueryEvents.buildingQuery, (args: IBuildingQueryEventArgs) => {
                this.options.fieldsToInclude.forEach((field: string) => {
                    const value: IAttributeWithValue = Insight.context.getAttributeValue(field);
                    if (value.value) {
            ...
            this.logger.warn(`No context attributes to include. Either remove the ${InsightContext.ID} component or include some fields.`);
        }
    }
    ...
    private getDynamicsModifiedData(field: string) {
        try {
            ...
            this.logger.warn(`An error occurred while parsing the parameter "${field}". Injecting raw value instead: ${exc.message}`);
            return field;
        }
    }
}

Initialization.registerAutoCreateComponent(InsightContext);

...
export class FormContextHandlerImpl {
    ...
    public setFormContext(context: IFormContext) {
        ...
        if (context && context.data && context.data.entity) {
            this.innerFormContextHandler = new EntityInnerFormContextHandler(context.data.entity);
        } else {
    ...
    public setAsyncFormContext(record: IRecord, entityName: string, id: string) {
        this.innerFormContextHandler = new ConstantInnerFormContextHandler(id, entityName);

        if (this.contextObjects) {
            this.contextObjects.register("form", new AsyncContextObject(record, id));
            this.contextObjects.register("record", new AsyncContextObject(record, id));
        }
    }
    ...
    get entityName(): string {
        return this.innerFormContextHandler ? this.innerFormContextHandler.getEntityName() : undefined;
    }

    get recordId(): UUID {
        return this.innerFormContextHandler ? this.innerFormContextHandler.getRecordId() : undefined;
    }
    ...
    getEntitySetName(webApi: WebApiService, entityLogicalName: string): Promise<string> {
        return webApi.init()

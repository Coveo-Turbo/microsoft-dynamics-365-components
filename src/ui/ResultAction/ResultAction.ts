...
export class ResultAction extends Component {
  ...
  static options: IResultActionOptions = {
     ...
     */
    menuDelay: ComponentOptions.buildNumberOption({
      defaultValue: 300,
      min: 0,
    }),
  };
  ...
  private renderParents(): void {
    const controlSquares = $$("div", {
      className: "coveo-result-action-squares",
    });
    for (let i = 1; i <= 3; i++) {
    ...
    this.container = $$("div", {
      className: "coveo-box-result-action-container",
    });
  ...
  private renderChildren(result?: IQueryResult): void {
    const replaceElementsOnce = _.once(() => {
      ...
      _.each(this.element.children, (child: HTMLElement) => {
        if (
          this.doesImplementIncludedInterface(child) &&
          Coveo.get(child)["display"]()
        ) {
          toMove.push(child);
        }
      });
      if (toMove.length > 0) {
        toMove.forEach((elem: HTMLElement) => {
          const menuItem = $$("div", {
            className: "coveo-box-result-action-menu-item",
          });
        ...
        this.logger.warn(
          "ResultAction is empty or has no inner elements with which it can populate... removing the component",
          result,
          this
        );
        $$(this.element).remove();

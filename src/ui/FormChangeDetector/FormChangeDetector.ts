...
import { AttributeType, ControlType } from "../../utils/CrmFormControlConstants";
...
/**
 * The _CoveoForDynamicsFormChangeDetector_ component triggers a new Coveo search query when a CRM attribute changes.
 */
export class ForDynamicsFormChangeDetector extends Component {
    static ID = "ForDynamicsFormChangeDetector";

    /**
     * The options for the ForDynamicsFormChangeDetector.
     * @componentOptions
     */
    public static options: IFormChangeDetectorOptions = {
         ...
         */
        enableSearchAsYouType: Coveo.ComponentOptions.buildBooleanOption({ defaultValue: false, section: "SearchAsYouType" }),
    };
     ...
     */
    constructor(public element: HTMLElement, public options: IFormChangeDetectorOptions, public bindings?: IComponentBindings) {
        super(element, ForDynamicsFormChangeDetector.ID, bindings);
    ...
    private initializeEvents(): void {
        FormContextHandler
            .controls
            .forEach(control => this.setListenerOnControl(control));
    }
    ...
    private setListenerOnControl(control: Xrm.Page.Control): void {
        if (this.isTextInputControl(control)) {
            if (this.options.enableSearchAsYouType) {
                (control as Xrm.Page.AutoLookupControl).addOnKeyPress(() => this.updateSearch());
            } else {
                (control as Xrm.Page.LookupControl).getAttribute().addOnChange(() => this.updateSearch());
            }
    ...
    private isTextInputControl(control: Xrm.Page.Control): boolean {
        return control.getControlType() === ControlType.Standard &&
               !!(control as Xrm.Page.StandardControl).getAttribute &&
               !!(control as Xrm.Page.StandardControl).getAttribute() &&
               (control as Xrm.Page.StandardControl).getAttribute().getAttributeType() === AttributeType.String;
    }
...
Initialization.registerAutoCreateComponent(ForDynamicsFormChangeDetector);
